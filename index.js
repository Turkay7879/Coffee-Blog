...
const selectImg = (img) => {
  ...
  while (count != 3) {
    ...
    if (
      photos[rand].path !== "../img/brew/" + img + ".jpg" &&
      !randImg.includes(photos[rand])
    ) {
...
const searchInData = async (searchedKey) => {
  ...
  for (let brewType in blogData.brewInstructions) {
    ...
    if (
      blogData.brewInstructions[brewType].intro
        .toLowerCase()
      ...
      if (
        searchResults.find((result) => result.srcKey === srcKey) === undefined
      ) {
        flag = "0";
  ...
  for (let country in blogData.variety) {
    ...
    if (
      blogData.variety[country].title
        .toLowerCase()
        .includes(searchedKey.toLowerCase())
    ) {
...
const coffeeInfo = async (coffeeKey) => {
  for (let i = 0; i < blogData.coffees.length; i++) {
    ...
    if (
      coffee.name.toLowerCase().includes(coffeeKey.toLowerCase()) ||
...
const countryInfo = async (countryKey) => {
  for (let i = 0; i < blogData.continents.length; i++) {
    ...
    if (
      country.name.toLowerCase().includes(countryKey.toLowerCase()) ||
...
app.post("/contact", async (req, res) => {
  const { isim, telefon, mail, mesaj } = req.body;
  const userMail = new Database.ContactMail({
    name: isim,
    phone: telefon,
    email: mail,
    message: mesaj,
